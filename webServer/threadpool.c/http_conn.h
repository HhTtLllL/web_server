...
class http_conn
{
	...
	private:
		...
		//客户请求的目标文件的完整路径,其内容等于 doc_root + m_url,doc_root 是网站的根目录
		...
		//湖库请求的目标文件被mmap到内存中的起始位置
		char* m_file_address;

		//目标文件的状态,通过它我们可以判断文件是否存在,是否为目录,是否刻度,并获取文件大小等信息
		struct stat m_file_stat;

		//我们将采用writev 来执行写操作,所以定义下面两个成员,其中 m_iv_count 表示被写内存快的数量
		struct iovec m_iv[2];
		int m_iv_count;
};
